...
local mainResearchTableFrame = player.gui.center[guiNames.mainResearchTableFlow][guiNames.mainResearchTableFrame]
	global.ResearchSystem.MaxShown = 0
	...
	local data = RSDatabase.ItemUnlock[RecipeName]
		if Research_System_Time_Usage then
			if global.ResearchSystem.science > data.Points and (data.Minute+(data.Hour*60)) < (remote.call("DyTech-Script", "Timer", "Minutes")+(remote.call("DyTech-Script", "Timer", "hours")*60)) and global.ResearchSystem.MaxShown <= (MaxRecipeShown-1) and data.Tier==TierRecipe then 
				mainResearchTableFrame[guiNames.RSTable].add({type="button", name=RecipeName, caption={data.Locale.."-name."..RecipeName}})
				global.ResearchSystem.MaxShown = global.ResearchSystem.MaxShown + 1
			end
		else
			if global.ResearchSystem.science > data.Points and global.ResearchSystem.MaxShown <= (MaxRecipeShown-1) and data.Tier==TierRecipe then
				mainResearchTableFrame[guiNames.RSTable].add({type="button", name=RecipeName, caption={data.Locale.."-name."..RecipeName}})
				global.ResearchSystem.MaxShown = global.ResearchSystem.MaxShown + 1
			end
		end
